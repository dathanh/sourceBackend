...
class Utils {
    ...
    public static function useTables($obj, $tableNames = []) {
        ...
        foreach ($tableNames as $tableName) {
            ...
            if (isset(BackendController::$_globalObjects['tables'][$tableClass]) && !empty(BackendController::$_globalObjects['tables'][$tableClass])) {
                $obj->$tableName = BackendController::$_globalObjects['tables'][$tableClass];
            } else {
                $table = TableRegistry::get($tableName);
                if (!empty($table)) {
                    $obj->$tableClass = BackendController::$_globalObjects['tables'][$tableClass] = $table;
                }
            }
    ...
    public static function useComponents($obj, $componentNames = []) {
        ...
        foreach ($componentNames as $componentName) {
            ...
            $controller = BackendController::$_instance;
            if (empty($controller)) {
                $controller = new BackendController(new \Cake\Network\Request(), new \Cake\Network\Response());
                $controller->initialize();
            }
            if (isset(BackendController::$_globalObjects['components'][$componentName]) && !empty(BackendController::$_globalObjects['components'][$componentName])) {
                $obj->$componentName = BackendController::$_globalObjects['components'][$componentName];
            } else {
                $pluginName = 'App';
                ...
                if (class_exists($componentClass)) {
                    ...
                    BackendController::$_globalObjects['components'][$componentName] = $obj->$componentName = $controller->$componentName;
                }
            }
    ...
    public static function cacheExistedComponents($obj) {
        if (!isset($obj->hasLoadComponents) && isset($obj->components) && is_array($obj->components) && !empty($obj->components)) {
            foreach ($obj->components as $key => $val) {
                ...
                if (is_string($componentName) && !empty($componentName) && isset($obj->$componentName) && !isset(BackendController::$_globalObjects['components'][$componentName])) {
                    BackendController::$_globalObjects['components'][$componentName] = $obj->$componentName;
                }
            }
